...
*/
    ...
    function voteYes(string memory _name) public {
        ...
        proposeVoters[_name][msg.sender] = true;
        addressList[_name][voters[_name]] = msg.sender;
        voters[_name] = voters[_name] + 1;
    ...
    function mint(string memory _name) public{
        ...
        for (uint j = 0; j >= voters[_name]; j ++) {  //for loop example
        
            address vots = addressList[_name][j];

                if(proposeVoters[_name][vots] == true){
                    
                    yesVootes = yesVootes + 1;
                }
        }

        if(yesVootes >= (voters[_name] / 2)){  
            supply = supply+amount;
            balances[founder] = balances[founder] + amount;
        }
